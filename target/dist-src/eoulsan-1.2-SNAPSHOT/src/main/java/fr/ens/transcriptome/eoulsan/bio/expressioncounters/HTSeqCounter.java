...
public class HTSeqCounter extends AbstractExpressionCounter {
  ...
  protected void internalCount(File alignmentFile, DataFile annotationFile,
    ...
    LOGGER.info("TESTS alignmentFile :\n"
        + alignmentFile.getAbsolutePath() + "\n"
        + alignmentFile.getCanonicalPath() + "\n" + alignmentFile.getParent());

    File samReport = new File(alignmentFile.getAbsolutePath(), "htseq-out.sam");
    File sam2Report = new File(alignmentFile.getParent(), "htseq-out.sam");
    LOGGER.info("TESTS samReport :\n"
        + samReport.getAbsolutePath() + "\n" + sam2Report.getAbsolutePath());
