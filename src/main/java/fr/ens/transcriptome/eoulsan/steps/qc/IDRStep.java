...
*/
@LocalOnly
public class IDRStep extends AbstractStep {
    ...
    /**
     * Value from the (narrow/broad)peak file used to rank the peaks. Can take three values: "signal.value", "p.value" or "q.value".
     */
    private String rankingMeasure = "p.value";

    /**
     * Genome assembly used for the mapping step. IDR needs to know the size of the chromosomes.
     * Consequently to this default initialization, if no 'assembly' parameter is set in the param.xml file, hg19 will be used by default.
     * A specific log message will warn the user. 
     */
    ...
    /**
     * Install IDR archive. Installation path will be memorized.
     * R needs to be installed and available.
     */
    private void install() {
        try {
            // Get the shipped archive
            String binaryFile = BinariesInstaller.install(this.TOOL_NAME,
            ...
            getLogger().warning("Error during IDR file installation : " + e.toString());
        }
    }
    ...
    public void configure(final StepConfigurationContext context, final Set<Parameter> stepParameters)
        ...
        for (Parameter p : stepParameters) {
            ...
            if ("min.overlap.ratio".equals(p.getName())) {
                ...
                if ("signal.value".equals(tmpRanking) || "p.value".equals(tmpRanking) || "q.value".equals(tmpRanking)) {
                    ...
                    throw new EoulsanException("Unknown value (" + p.getStringValue() + ") for ranking.measure parameter (step: " + getName() + ").");
                }
            }
        ...
        if(this.assembly.equals("")) {
            getLogger().warning("No assembly specified. Will use human genome (version 19) by default");
        }
...
//         getLogger().info("All input data : " + inData.getMetadata()); // empty?
        ...
        if(!inData.isList() || inData.getListElements().size()<2) {
            getLogger().severe("Not enough data to run IDR. Need a list of at least two samples.");
...
//             getLogger().info("One input file : " + anInputData.getMetadata().toString());
            getLogger().info("One input file. ref : " + anInputData.getMetadata().get("Reference") + 
            ...
            getLogger().info("Now " + expMap.get(experimentName).size() + " samples for experiment " + experimentName);
        }
        ...
        for (String experimentName : expMap.keySet()) {
            ...
            HashMap<String, ArrayList<Data>> replicatesMap = new HashMap<String, ArrayList<Data>>(expDataList.size()/2);
            ...
            for (Map.Entry <String, ArrayList<Data>> entry : replicatesMap.entrySet()) {
                ...
                if(replicatesList==null || replicatesList.size()==0) {
                    getLogger().severe("Incoherence : experiment " + entry.getKey() + " has no corresponding sample.");
                }
                ...
                if(nbSameReplicates==1) {
                    ...
                    getLogger().info("Will not call IDR on 1 sample, for experiment " + tmp.getMetadata().get("Experiment") +
                                     " in replicate group " + tmp.getMetadata().get("RepTechGroup"));
                    continue;
                }
                
                String outputDir = "";
                try{
                    outputDir = replicatesList.get(0).getDataFile().getParent().getSource();
                } catch (java.io.IOException e){
                    getLogger().severe("Error while accessing parent folder of " + replicatesList.get(0).getDataFile().getSource() + ". " + e.toString());
                }
                ...
                for (int i = 0; i < nbSameReplicates; i++) {
                    for (int j = i + 1; j < nbSameReplicates; j++) {
                        ...
                        if(retVal==0) { 
                            outputs.add(String.format("%s/idr_output_%s_vs_%s", 
                                                      outputDir, 
    ...
    private int runAnalysis(Data data1, Data data2) {
        ...
        if (!data1.getDataFile().exists()) { getLogger().severe("File " + data1.getDataFile().getSource() + " doesn't exist."); return -1; }
        if (!data2.getDataFile().exists()) { getLogger().severe("File " + data2.getDataFile().getSource() + " doesn't exist."); return -1; }

        String outputDir = "";
        try{
            outputDir = data1.getDataFile().getParent().getSource();
        } catch (java.io.IOException e){
            getLogger().severe("Error while accessing parent folder of " + data1.getDataFile().getSource() + ". " + e.toString());
            return -1;
        }
            
        // As IDR is "well written", it sources files that are located inside its own folder. So we must move to that folder first in order for IDR to find them...
        ...
        String cmd = String.format("cd %s/idrCode/ ; Rscript %s/batch-consistency-analysis.r %s %s %d %s/%s %d %s %s %s; cd -",
            this.idrPath, 
            ".", 
        ...
        // Check that the output string contains the output of the "cd -" i.e. path of the output folder
...
//         }
        ...
        return runAnalysisPlot(1, 
                        String.format("%s/%s", outputDir, outputPrefix),
                        Arrays.asList(String.format("%s/%s", outputDir, inputPrefix)));
    }
    
    /**
    * Buil plot from one or more analyses.
    * usage: 
    * Rscript batch-consistency-plot-merged.r [npairs] [output.file.prefix] [input.file.prefix 1, 2, 3 ...]
    * [npairs]: integer, number of consistency analyses
    *          (e.g. if 2 replicates, npairs=1, if 3 replicates, npairs=3
    * [output.file.prefix]: output prefix for plot
    * [input.file.prefix 1, 2, 3]: prefix for the output from batch-consistency-analysis2. They are the input files for merged analysis. It can be multiple files
    */
    private int runAnalysisPlot(int nbPairs, String outputPrefix, Iterable<String> inputPrefixIter) {
        ...
        String cmd = String.format("cd %s/idrCode/ ; Rscript %s/batch-consistency-plot.r %d %s %s; cd -",
