...
public final class ProcessUtils {
	...
	public static int system(final String cmd) throws IOException {
		...
		logger.fine("execute (Thread " + Thread.currentThread().getId() + "): "
				+ cmd);
	 ...
	 *            temporary where create the temporary shell file
	 * @return the exit error of the program
	 * @throws IOException
	...
	public static int sh(final List<String> cmd, final File temporaryDirectory)
		...
		try {
			...
			final Thread terr = new Thread(new ProcessThreadErrOutput(
					p.getErrorStream()));
			terr.start();
	...
	private static ProcessResult shWithOutputs(final String cmd,
		...
		logger.fine("execute script (Thread " + Thread.currentThread().getId()
				+ "): " + cmd);
		...
		try {
			...
			final BufferedReader stdr = new BufferedReader(
					new InputStreamReader(std, CHARSET));
			...
			if (!f.delete())
				logger.warning("Can not remove sh script: "
						+ f.getAbsolutePath());

			// Return result
			return new ProcessResult(exitValue, stdout.toString(),
					stderr.toString());
	 ...
	 *            Command to execute
	 * @param stdOutput
	 *            don't show the result of the command on the standard output
	 * @throws IOException
	 ...
	 *            Command to execute
	 * @param addStdErr
	 *            add the output of stderr in the result
	 * @return a string with the output the command
	 * @throws IOException
	 *             if an error occurs while running the process
	 */
	public static String execToString(final String cmd,
			final boolean addStdErr, final boolean checkExitCode)
			throws IOException {
		...
		final BufferedReader stdr = new BufferedReader(new InputStreamReader(
				std, CHARSET));
	 ...
	 *            Start time in ms
	 * @throws IOException
	 *             if an error occurs at the end of the process
	 */
	public static final void logEndTime(final Process p, final String cmd,
		...
		try {
			...
			logger.fine("Done (Thread " + Thread.currentThread().getId()
					+ ", exit code: " + exitValue + ") in "
	...
	public static Set<Integer> getExecutablePids(final String executableName) {
		...
		try {
			final String s = ProcessUtils.execToString("pgrep "
					+ executableName.trim());
  ...
  public static final class ProcessThreadErrOutput implements Runnable {
	...
	/**
	 * Execute a command and write the content of the standard output and error
	...
	public static void execThreadOutput(final String cmd) throws IOException {
		...
		final BufferedReader errr = new BufferedReader(new InputStreamReader(
	 ...
	 * 
	 * @param cmd
	 *            array with the command to execute
	 * @throws IOException
	 *             if an error occurs while executing the command
	 */
	public static void execThreadOutput(final String[] cmd) throws IOException {
