...
public class TrimAdaptModule extends AbstractModule {
  ...
  // Adapter sequence:
  // auto try to auto-detect adapter sequence, if it can't it defaults to
  // illumina
  ...
  public void configure(final StepConfigurationContext context,
    ...
    for (Parameter p : stepParameters) {
      ...
      getLogger().info(
          "TrimAdapt parameter: " + p.getName() + " : " + p.getStringValue());
      ...
      if ("phred.cutoff".equals(p.getName())) {
        ...
        if (p.getDoubleValue() < 0 && (p.getDoubleValue() % 1) != 0) {
          throw new EoulsanException("Invalid Phred score cutoff for parameter "
              + p.getName() + " step: " + getName());
        }
      ...
      // illumina Adapter sequence to be trimmed is the first 13bp of the
      // Illumina universal adapter
      // 'AGATCGGAAGAGC' instead of the default auto-detection of adapter
      // sequence.
      // nextera Adapter sequence to be trimmed is the first 12bp of the Nextera
      // adapter
      // 'CTGTCTCTTATA' instead of the default auto-detection of adapter
      // sequence.
      // small_rna Adapter sequence to be trimmed is the first 12bp of the
      // Illumina Small RNA Adapter
      // 'ATGGAATTCTCG' instead of the default auto-detection of adapter
      // sequence.
      // [ACGTacgt] Any base sequence
        ...
        if (adaptVal.equals("auto")) { // auto-detect. Default behavior.
          ...
          for (char c : a) {
            valid = (c == 'a')
                ...
                || (c == 'C') || (c == 'G') || (c == 'T');
            if (!valid) {
              throw new EoulsanException("Invalid character "
                  + c + " in adapter sequence for parameter " + p.getName()
                  + " step: " + getName());
            }
          }
      ...
      // Maximum trimming error rate: 0.1 (default)
        ...
        if (p.getDoubleValue() < 0) {
          throw new EoulsanException(
              "Invalid maximum trimming error rate for parameter "
                  + p.getName() + " step: " + getName());
        }
      ...
      // Minimum required adapter overlap (stringency): 1 bp
        ...
        if (p.getDoubleValue() < 0 && (p.getDoubleValue() % 1) != 0) {
          throw new EoulsanException(
              "Invalid minimum required adapter overlap (stringency) for parameter "
                  + p.getName() + " step: " + getName());
        }
      ...
      // Minimum required sequence length before a sequence gets removed: 20 bp
        ...
        if (p.getDoubleValue() < 0 && (p.getDoubleValue() % 1) != 0) {
          throw new EoulsanException(
              "Invalid minimum required sequence length for parameter "
                  + p.getName() + " step: " + getName());
        }
  ...
  /**
   * Check whether cutadapt and Trim_galore are already installed. If not
   * decompress Eoulsan's included archives and install them.
   */
  private void install() {

    // Install cutadapt
    if (!BinariesInstaller.check(SOFTWARE_LABEL_CA, SHIPPED_PACKAGE_VERSION_CA,
        "cutadapt")) {
      ...
      try {
        ...
        String binaryFile = BinariesInstaller.install(SOFTWARE_LABEL_CA,
            SHIPPED_PACKAGE_VERSION_CA, PACKAGE_ARCHIVE_CA, EoulsanRuntime
                .getSettings().getTempDirectoryFile().getAbsolutePath());
        getLogger().info("Archive location for cutadapt : " + binaryFile);
        DataFile cutadaptArchive = new DataFile(binaryFile);

        // Extract full archive
        String cmd =
            String.format("tar -xzf %s -C %s", cutadaptArchive.getSource(),
                cutadaptArchive.getParent().getSource());
        getLogger().info("Extract archive : " + cmd);
        ProcessUtils.system(cmd);

        // Memorize path
        this.cutadaptPath =
            cutadaptArchive.getParent() + "/cutadapt-1.8.1/bin/cutadapt";

      } catch (java.io.IOException e) {
        getLogger()
            .severe("Error during cutadapt installation : " + e.toString());
        return;
      }
    }

    // Install Trim galore!
    if (!BinariesInstaller.check(SOFTWARE_LABEL_TG, SHIPPED_PACKAGE_VERSION_TG,
        "trim_galore")) {
      ...
      try {
        // Get the shipped archive
        String binaryFile = BinariesInstaller.install(SOFTWARE_LABEL_TG,
            SHIPPED_PACKAGE_VERSION_TG, PACKAGE_ARCHIVE_TG, EoulsanRuntime
                .getSettings().getTempDirectoryFile().getAbsolutePath());
        getLogger().info("Archive location for trim_galore : " + binaryFile);
        DataFile trimgArchive = new DataFile(binaryFile);

        // Extract full archive
        // Note that unzip must be in very quiet mode otherwise the process
        // doesn't terminate
        ...
        // Memorize path
        this.trimGalorePath =
            trimgArchive.getParent() + "/trim_galore_zip/trim_galore";

      } catch (java.io.IOException e) {
        getLogger()
            .severe("Error during trim_galore installation : " + e.toString());
        return;
      }
  ...
  /**
   * Run cutadapt through trim_galore. Installation (if needed) was made during
   * configuration.
   */
  @Override
  public TaskResult execute(final TaskContext context,
    ...
    getLogger().info("Running trim_galore "
        + this.trimGalorePath + " using cutadapt " + this.cutadaptPath);
    ...
    try {
      ...
      getLogger()
          .severe("Could not execute " + cmd + ". Error:" + e.toString());
      return status.createTaskResult();
    }

    //// Rename the output files to be complient with the naming scheme of
    //// Eoulsan (thus making them available to further analysis steps)

    // First, create a Datafile with the output file name of trim_galore
    final DataFile tmpTrimmedFile =
        new DataFile(StringUtils.filenameWithoutCompressionExtension(
            inData.getDataFile().getSource()) + "_trimmed.fq");
    ...
    if (tmpTrimmedFile.exists()) {
      ...
      // Rename report
      new DataFile(inData.getDataFile().getSource() + "_trimming_report.txt")
          .toFile().renameTo(new File(
              outputFile.getDataFile().getSource() + "_trimming_report.txt"));
    }
