...
if(length(target$counts[1,]) >= 3){
# compute clustering on unormalized counts
repClust(target)
} else {
	print("no clustering  because there is less than 3 samples")
}
@
...
if(length(normTarget$counts[1,]) >= 3){
	repClust(normTarget)
} else {
	print("no clustering  because there is less than 3 samples after pooling")
}
@
...
saveCountMatrix(
		...
		outPath,
		# file name
		paste("anaDiff_", projectName, "_normalizedPooledCountMatrix.txt", sep="")
)
@
\end{itemize}