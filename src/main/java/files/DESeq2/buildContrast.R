...
for(comparisonRow in 1:nrow(comparisonFile)){
    ...
    # data frame kipping the first row without the "condition" column to kip 
    # only column corresponding to the contrast vector. The first line kip 
            ...
            for(j in 2:length(vectorsToAssociate)){
                ...
                if(test %in% betaFactorDFrame$condition){
                    ...
                    associatedVector <- associatedVector + 
						subset(betaFactorDFrame, condition == test)
						[2:length(names(betaFactorDFrame))]
                }
            ...
            if(length(sameColumnFactorNames)>0){
                ...
                for( p in 1:length(sameColumnFactorNames)){
                    ...
                    betaFactorVector<- subset(betaFactorDFrame, 
						condition == sameColumnFactorNames[p])
						[2:length(names(betaFactorDFrame))]
...
# Remove of the first row of the contrastData that was kip during its 
# construction
